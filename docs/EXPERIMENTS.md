...
|---------|-----|--------|-------|
| **v6.0-e1-complete-3-arm** | `v6.0-e1-cross-env-eval-3arm` | **Complete 3-arm E1 ablation. Order matters strongly (51.7pp gap best vs worst), but NOT in the developmental direction. Reverse (hard→easy): 98/55/15 mean **56.00%** wins. Forward (easy→hard): 97/31/9 mean **45.67%**. Shuffled (easy→hard→medium): 13/0/0 mean **4.33%** — catastrophic collapse on greedy eval despite reasonable training window_R (0.476 on GoToLocal final stage). Mechanism hypothesis: shuffled's action-space whiplash (3 actions → 4 actions → 3 actions across stages) breaks the policy in a way that aligned-ordering arms (forward, reverse) avoid. The substrate's freeze mechanism cannot protect against poorly-ordered curricula. Findings: (1) developmental easy-to-hard ordering hypothesis FALSIFIED — reverse outperforms forward by 10.3pp. (2) Curriculum order IS strongly load-bearing — just not in the easy-to-hard direction. (3) Hard-first ordering preserves the difficult capability AND transfers downward better than easy-first transfers upward. (4) "Scrambled" order with action-space changes mid-curriculum produces catastrophic policy collapse on greedy eval. Substrate framing: not "developmental cognition" but "continual-learning architecture where curriculum order respecting task structure (action-space alignment, hard-first) is load-bearing for transfer." Caveats: single seed per arm, n=100 per cell. 51.7pp gap between best and worst arms is well above sampling noise.** |
...
### Headline finding (E1 — complete 3-arm ablation)

**The "easy-to-hard developmental ordering matters" hypothesis is empirically refuted, AND a stronger principle emerges: curriculum order is strongly load-bearing (51.7pp gap best vs worst), but the load-bearing dimension is not "developmental" — it's "respects task structure" (action-space alignment + hard-first).**

Complete 3-arm cross-env evaluation at 100 greedy episodes per (arm, env) cell:

| Arm | GoToObj | GoToLocal | PickupLoc | Mean | Stage order |
|-----|--------:|----------:|----------:|-----:|-------------|
| **reverse** | 98% | **55%** | **15%** | **56.00%** | action composition (PickupLoc) → object recognition (GoToLocal) → sensorimotor (GoToObj) |
| forward | 97% | 31% | 9% | 45.67% | sensorimotor → object recognition → action composition |
| **shuffled** | **13%** | **0%** | **0%** | **4.33%** | sensorimotor (GoToObj) → action composition (PickupLoc) → object recognition (GoToLocal) |

The 51.7pp gap between best (reverse) and worst (shuffled) demolishes any claim that "order doesn't matter." The v6 plan called the original falsifier:

> "if shuffled ≈ forward at matched per-stage budgets, 'developmental' is not load-bearing — drop the developmental framing."

The actual result is stronger and more nuanced: **shuffled << forward, and reverse > forward.** Three findings:

1. **The naive "developmental easy-to-hard" hypothesis is refuted.** Reverse (hard-first) beats forward (easy-first) by 10.3pp mean. Hard-first preserves the difficult capability (PickupLoc retention 15% vs 9%) AND transfers downward (GoToLocal 55% vs 31%) better than easy priors transfer upward.

2. **Curriculum order IS strongly load-bearing — but in a structural sense, not a developmental sense.** A "scrambled" order that happens to violate task structure (shuffled: easy → hard → medium, with action-space changes) catastrophically destroys the policy on greedy eval. Shuffled's training window_R reached 0.476 on GoToLocal final stage but greedy eval gives 0%. The policy is sampling-dependent: stochastic rollouts find the goal sometimes, but the argmax-of-policy doesn't.

3. **Mechanism hypothesis: action-space whiplash.** Forward: 3-action (at) → 3-action (at) → 4-action (holding). Reverse: 4-action (holding) → 3-action (at) → 3-action (at). Both have at most ONE action-space transition. Shuffled: 3-action (GoToObj) → 4-action (PickupLoc) → 3-action (GoToLocal) — TWO transitions, with action 3 "added then masked away." This breaks the policy distribution in a way the substrate's freeze mechanism does not protect against. The freeze mechanism locks weights but cannot lock the action-mask-dependent probability redistribution that PPO learns.

**Implications for PRISM positioning:**
- Drop "developmental cognition" — empirically falsified.
- Adopt **"continual-learning architecture where curriculum order respecting task structure is load-bearing for transfer."** Stronger claim, scientifically defensible.
- For deployment guidance: curricula should be designed hard-first when possible, and should NOT mix action-space-changing transitions with non-aligned task ordering.
- The "freeze mechanism prevents catastrophic forgetting" claim only holds when curriculum order is structurally sensible. Bad ordering can defeat any continual-learning architecture.

**Caveats (load-bearing):** single seed per arm; n=100 episodes per cell (95% CI ±10pp on individual cells). The 51.7pp best-vs-worst gap is well above noise. The 10.3pp forward-vs-reverse gap is at the single-seed noise floor and would need 3-5 seeds to harden. The action-space-whiplash mechanism is a hypothesis that explains the shuffled collapse but has not been independently isolated (would need a controlled experiment fixing all-3-action curricula).
